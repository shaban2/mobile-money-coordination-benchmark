...
artifact_ref = f' Zenodo, 2026. https://doi.org/{ARTIFACT_DOI} (all versions).' if ARTIFACT_DOI else ' GitHub, 2026.'
artifact_cite = (f'archived at Zenodo under the all-versions identifier https://doi.org/{ARTIFACT_DOI}' if ARTIFACT_DOI else 'with an archival identifier pending')
...
para('Nadeem and Malik port the TrainTicket benchmark to the Temporal workflow engine, report initial findings on debugging time, and identify fuller evaluation as future work [8]; we adopt the same early-results positioning. Hegde et al. compare synchronous and asynchronous microservice implementations using k6, Docker resource monitoring, and worker termination, and report API-response performance [9]. Our endpoint is durable ledger completion rather than API response, and both of our conditions expose an asynchronous client contract. Aydın and Çebi examine choreography and orchestration Saga implementations with isolated NoSQL databases and cross-service rollback [10]; our common ledger is not that setting. The outbox side of our Kafka condition follows the transactional outbox pattern described by Richardson [11]; the inbox, which records processed message identifiers to discard duplicates, follows his idempotent-consumer approach [11].')
para('Hasselbring argues that useful benchmarking requires explicit workloads, fairness, verifiability, and a replication package [4]. Kalibera and Jones emphasize repetition at the level where performance varies [5]. These considerations motivate our documented scope and our refusal to treat thousands of transfers within a run as thousands of independent architecture experiments.')
...
para('k6\'s constant-arrival-rate executor starts operations at a fixed rate regardless of response time, provided enough pre-allocated virtual users remain [6]. An operation is either a creation attempt or a status read, not necessarily a new transfer. The repeating 100-operation schedule contains 76 unique creations, four idempotent creation replays, and 20 status reads. Thus replays are 5% of creation attempts and 4% of all operations; these proportions are benchmark choices rather than estimates from customer traffic.')
...
para('Resource summaries sum the application, gateway, PostgreSQL, Toxiproxy, adapter, callback receiver, Prometheus, and Grafana, plus Redpanda for Kafka. They exclude k6, the controller, and host overhead. Memory is the usage reported by the Docker CLI, which excludes cache on Linux [7]; for CPU, we sum the per-container percentages and divide by 100. Means are time-weighted over samples about 2 s apart within measurement; a stopped adapter contributes zero while absent. These are descriptive footprints, not cost or reserved capacity.')
...
references = [
    ...
    '[4] W. Hasselbring, “Benchmarking as Empirical Standard in Software Engineering Research,” in Proc. Evaluation and Assessment in Software Engineering (EASE), pp. 365–372, 2021. doi: 10.1145/3463274.3463361.',
    '[5] T. Kalibera and R. Jones, “Rigorous Benchmarking in Reasonable Time,” in Proc. International Symposium on Memory Management (ISMM), pp. 63–74, 2013. doi: 10.1145/2464157.2464160.',
    '[6] Grafana Labs, “Constant arrival rate,” k6 documentation. Accessed Sep. 21, 2026. https://grafana.com/docs/k6/latest/using-k6/scenarios/executors/constant-arrival-rate/',
    '[7] Docker, “docker container stats,” Docker documentation. Accessed Sep. 21, 2026. https://docs.docker.com/reference/cli/docker/container/stats/',
    '[8] A. Nadeem and M. Z. Malik, “A Case for Microservices Orchestration Using Workflow Engines,” in Proc. IEEE/ACM 44th Int. Conf. Software Engineering: New Ideas and Emerging Results (ICSE-NIER), pp. 6–10, 2022. doi: 10.1145/3510455.3512777.',
    '[9] P. Hegde N et al., “An Empirical Evaluation of Synchronous vs Asynchronous Microservice Architectures for Enterprise Messaging Platforms,” in Proc. Int. Conf. Artificial Intelligence and Data Engineering (AIDE), pp. 663–666, 2026. doi: 10.1109/AIDE69088.2026.11545016.',
    '[10] S. Aydın and C. B. Çebi, “Comparison of Choreography vs Orchestration Based Saga Patterns in Microservices,” in Proc. Int. Conf. Electrical, Computer and Energy Technologies (ICECET), pp. 1–6, 2022. doi: 10.1109/ICECET55527.2022.9872665.',
    '[11] C. Richardson, Microservices Patterns: With Examples in Java. Shelter Island, NY, USA: Manning, 2018.',
    '[12] M. Bianchi, M. Bouvard, R. Gomes, A. Rhodes, and V. Shreeti, “Mobile payments and interoperability: Insights from the academic literature,” Information Economics and Policy, vol. 65, art. 101068, 2023. doi: 10.1016/j.infoecopol.2023.101068.',
    '[13] Committee on Payments and Market Infrastructures, “Promoting the harmonisation of application programming interfaces to enhance cross-border payments: Recommendations and toolkit,” Bank for International Settlements, Basel, 2024. https://www.bis.org/cpmi/publ/d224.htm',
    '[14] GSMA, “Mobile Money API Specification 1.2.0: Fundamentals,” GSMA, London, 2021. https://www.gsma.com/mobilefordevelopment/wp-content/uploads/2021/10/Mobile-Money-API-Specification-1.2.0-Fundamentals.pdf',
    '[15] Mojaloop Foundation, “Mojaloop Hub,” Mojaloop documentation, 2022. https://docs.mojaloop.io/technical/overview/',
    f'[16] S. Lubanga, “Mobile-money coordination benchmark: Code, redacted evidence, analysis and paper,” {ARTIFACT_VERSION},{artifact_ref} {ARTIFACT_URL}'
]
